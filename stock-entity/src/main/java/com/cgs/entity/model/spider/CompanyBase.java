package com.cgs.entity.model.spider;

import java.util.Date;

public class CompanyBase implements Comparable<CompanyBase>{
  ...
  public CompanyBase parseFromRedis(String company){
    CompanyBase companyBase = new CompanyBase();
    return companyBase;
  }

  @Override
  public int compareTo(CompanyBase o) {
    long currentMills = this.getDate().toInstant().toEpochMilli();
    long targetMills = o.getDate().toInstant().toEpochMilli();
    if (currentMills > targetMills){
      return 1;
    }else {
      return -1;
    }
